...
![](./image/第15章—如何创建动态模块-3.png)

这个模块是静态的，也就是它的内容是固定不变的，每次 import 都是一样：

![](./image/第15章—如何创建动态模块-4.png)

![](./image/第15章—如何创建动态模块-5.png)
...
![](./image/第15章—如何创建动态模块-18.png)

这样这个 CccModule 就已经有了 register 和 registerAsync 方法了：

![](./image/第15章—如何创建动态模块-19.png)

不用自己定义了，省事了不少。

传入 options：

![](./image/第15章—如何创建动态模块-20.png)

那现在如何在 Module 内注入这个 options 呢？

记得 build class 的时候返回了一个 token 么？

![](./image/第15章—如何创建动态模块-21.png)

就用这个注入：

![](./image/第15章—如何创建动态模块-22.png)
...
![](./image/第15章—如何创建动态模块-25.png)

![](./image/第15章—如何创建动态模块-26.png)
...
![](./image/第15章—如何创建动态模块-27.png)

这样创建的就是全局的模块。

不过这样还有个问题：

![](./image/第15章—如何创建动态模块-28.png)

options 那里多了 isGlobal 属性，但是类型定义这里还没有呀。

因为我们用的是这个类型：

![](./image/第15章—如何创建动态模块-29.png)

最好是用 builder 返回的类型：

![](./image/第15章—如何创建动态模块-30.png)

这样就有了：

![](./image/第15章—如何创建动态模块-31.png)

而这个 ASYNC\_OPTIONS\_TYPE 是 async 方式创建模块的 otpion 类型：

![](./image/第15章—如何创建动态模块-32.png)
