...
![](./image/第17章—Nest的Middleware-3.png)
...
![](./image/第17章—Nest的Middleware-4.png)

可以看到中间件的逻辑都执行了：

![](./image/第17章—Nest的Middleware-5.png)

这里也可以指定更精确的路由。

我们添加几个 handler：

![](./image/第17章—Nest的Middleware-6.png)

然后重新指定 Middleware 应用的路由：

![](./image/第17章—Nest的Middleware-7.png)

可以看到，hello、hello2、world2 的路由都调用了这个中间件，而 world1 没有：

![](./image/第17章—Nest的Middleware-8.png)
...
![](./image/第17章—Nest的Middleware-9.png)

当然，这里也可以用构造器注入，这样更简洁一点：

![](./image/第17章—Nest的Middleware-10.png)

这时在访问这个路由的时候，就可以看到中间件成功调用了 AppService：

![](./image/第17章—Nest的Middleware-11.png)
...
![](./image/第17章—Nest的Middleware-12.png)
...
![](./image/第17章—Nest的Middleware-15.png)

但如果是这样一个 handler，它就不返回值了：

![](./image/第17章—Nest的Middleware-16.png)
...
![](./image/第17章—Nest的Middleware-17.png)

这个上节讲过。

当然，传入 Next 参数的时候，一般是不需要在这里响应的，一般是调用下个 handler 来响应：

![](./image/第17章—Nest的Middleware-18.png)

![](./image/第17章—Nest的Middleware-19.png)
...
![](./image/第17章—Nest的Middleware-20.png)
...
![](./image/第17章—Nest的Middleware-21.png)
