...
![](./image/第11章—ExecutionContext：切换不同上下文-1.png)

然后创建一个 filter：

    nest g filter aaa --flat --no-spec

![](./image/第11章—ExecutionContext：切换不同上下文-2.png)

![](./image/第11章—ExecutionContext：切换不同上下文-3.png)
...
![](./image/第11章—ExecutionContext：切换不同上下文-4.png)

在 @Catch 装饰器里声明这个 filter 处理该异常：

![](./image/第11章—ExecutionContext：切换不同上下文-5.png)

然后需要启用它：

![](./image/第11章—ExecutionContext：切换不同上下文-6.png)

路由级别启用 AaaFilter，并且在 handler 里抛了一个 AaaException 类型的异常。

也可以全局启用：

![](./image/第11章—ExecutionContext：切换不同上下文-7.png)

访问 <http://localhost:3000> 就可以看到 filter 被调用了。

![](./image/第11章—ExecutionContext：切换不同上下文-8.png)

filter 的第一个参数就是异常对象，那第二个参数呢？

可以看到，它有这些方法：

![](./image/第11章—ExecutionContext：切换不同上下文-9.png)

我们用调试的方式跑一下：

点击 create launch.json file 创建一个调试配置文件：

![](./image/第11章—ExecutionContext：切换不同上下文-10.png)
...
![](./image/第11章—ExecutionContext：切换不同上下文-11.png)

打个断点：

![](./image/第11章—ExecutionContext：切换不同上下文-12.png)

浏览器访问 <http://localhost:3000> 就可以看到它断住了：

![](./image/第11章—ExecutionContext：切换不同上下文-13.png)

我们分别调用下这些方法试试：

在 debug console 输入 host，可以看到它有这些属性方法：

![](./image/第11章—ExecutionContext：切换不同上下文-14.png)
...
![](./image/第11章—ExecutionContext：切换不同上下文-15.png)

这种按照下标取参数的写法不太建议用，因为不同上下文参数不同，这样写就没法复用到 ws、tcp 等上下文了。

一般是这样来用：

![](./image/第11章—ExecutionContext：切换不同上下文-16.png)
...
![](./image/第11章—ExecutionContext：切换不同上下文-17.png)

![](./image/第11章—ExecutionContext：切换不同上下文-18.png)
...
![](./image/第11章—ExecutionContext：切换不同上下文-19.png)

可以看到它传入的是 ExecutionContext：

![](./image/第11章—ExecutionContext：切换不同上下文-20.png)

有这些方法：

![](./image/第11章—ExecutionContext：切换不同上下文-21.png)

是不是很眼熟？

没错，ExecutionContext 是 ArgumentHost 的子类，扩展了 getClass、getHandler 方法。

![](./image/第11章—ExecutionContext：切换不同上下文-22.png)
...
![](./image/第11章—ExecutionContext：切换不同上下文-23.png)

在 Guard 里打个断点：

![](./image/第11章—ExecutionContext：切换不同上下文-24.png)

调用下 context.getClass 和 getHandler：

![](./image/第11章—ExecutionContext：切换不同上下文-25.png)

![](./image/第11章—ExecutionContext：切换不同上下文-26.png)
...
![](./image/第11章—ExecutionContext：切换不同上下文-27.png)

然后定义这样一个装饰器：

![](./image/第11章—ExecutionContext：切换不同上下文-28.png)

它的作用是往修饰的目标上添加 roles 的 metadata。

然后在 handler 上添加这个装饰器，参数为 admin，也就是给这个 handler 添加了一个 roles 为 admin 的metadata。

![](./image/第11章—ExecutionContext：切换不同上下文-29.png)
...
![](./image/第11章—ExecutionContext：切换不同上下文-30.png)
...
![](./image/第11章—ExecutionContext：切换不同上下文-31.png)

![](./image/第11章—ExecutionContext：切换不同上下文-32.png)
