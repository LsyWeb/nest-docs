...
![](./image/第55章—Nest里实现Session和JWT-1.png)

然后用 postman 测试下：

![](./image/第55章—Nest里实现Session和JWT-2.png)
...
![](./image/第55章—Nest里实现Session和JWT-3.png)
...
![](./image/第55章—Nest里实现Session和JWT-4.png)
...
![](./image/第55章—Nest里实现Session和JWT-5.png)
...
![](./image/第55章—Nest里实现Session和JWT-6.png)

放到请求的 header 里：

![](./image/第55章—Nest里实现Session和JWT-7.png)

这时候响应为 2，并且返回一个新的 token：

![](./image/第55章—Nest里实现Session和JWT-8.png)

把它复制下来放到 header 里再次请求：

![](./image/第55章—Nest里实现Session和JWT-9.png)
...
![](./image/第55章—Nest里实现Session和JWT-10.png)

这时候 jwtService.verify 方法就会抛异常，然后我们返回了 401 错误。

![](./image/第55章—Nest里实现Session和JWT-11.png)
