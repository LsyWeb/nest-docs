...
![](http://static.liushuaiyang.com/nest-docs/image/第97章-1.png)

现在，用户端首页是这样的：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-2.png)

需要在 / 下添加一个二级路由：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-3.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第97章-4.png)
...
npm run start:dev
```
![](http://static.liushuaiyang.com/nest-docs/image/第97章-5.png)

点击菜单项的路由切换，以及刷新选中对应菜单项，都没问题。

然后来写下列表页面，其实这个和管理端的会议室列表差不多：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-6.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第97章-7.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第97章-8.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第97章-9.png)

其实会议室的接口都是需要登录才能访问的，当时为了测试方便没有加，现在加一下：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-10.png)

添加 @RequireLogin 装饰器，标识接口需要登录。

并且添加对应的 @ApiBearerAuth 的 swagger 装饰器，代表需要添加 Bearer 的 header。

![](http://static.liushuaiyang.com/nest-docs/image/第97章-11.png)

我们现在 postman 里测试下：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-12.png)

这时候直接调用 delete 接口就会提示需要先登录了。

然后我们登录下，拿到 token。

![](http://static.liushuaiyang.com/nest-docs/image/第97章-13.png)

把它复制到 swagger 文档这里： 

![](http://static.liushuaiyang.com/nest-docs/image/第97章-14.png)

然后点击这个 try it out：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-15.png)

数据库中现在有 3 条记录：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-16.png)

把 id 为 10 那条删掉。

点击 execute：
![](http://static.liushuaiyang.com/nest-docs/image/第97章-17.png)

swagger 会发送请求，下面会打印响应：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-18.png)

这时数据库里就没有这条记录了：
![](http://static.liushuaiyang.com/nest-docs/image/第97章-19.png)

可以直接在 swagger 文档里测试接口，不用 postman 也行。

然后继续写下个接口的 swagger 文档：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-20.png)

这个接口的参数也是用 @ApiParam 标识，但它的响应不是 string，而是 MeetingRoom。

而我们现在并没有 vo，没地方标识属性：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-21.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第97章-22.png)
```javascript
@ApiBearerAuth()
@ApiParam({
    name: 'id',
...
![](http://static.liushuaiyang.com/nest-docs/image/第97章-23.png)

接下来是 update 接口：

他有两种响应：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-24.png)

分别写一下：
![](http://static.liushuaiyang.com/nest-docs/image/第97章-25.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第97章-26.png)

因为 update 的 dto 继承了 create 的 dto，所以那里也要加一下：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-27.png)

这样 swagger 文档就对了：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-28.png)

然后是 create 接口：

postman 里调用下是这样的：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-29.png)

![](http://static.liushuaiyang.com/nest-docs/image/第97章-30.png)

所以 swagger 装饰器这样写：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-31.png)
```javascript
@ApiBearerAuth()
@ApiBody({
...
![](http://static.liushuaiyang.com/nest-docs/image/第97章-32.png)

然后还有最后一个 list 接口：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-33.png)

它的响应是这样的：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-34.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第97章-35.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第97章-36.png)

![](http://static.liushuaiyang.com/nest-docs/image/第97章-37.png)

最后，在 controller 上加上个 @ApiTags，把下面的接口分到单独一组：

![](http://static.liushuaiyang.com/nest-docs/image/第97章-38.png)

![](http://static.liushuaiyang.com/nest-docs/image/第97章-39.png)
