...
![](./image/第07章—使用多种Provider，灵活注入对象-1.image#?w=848&h=686&s=275790&e=png&b=020202.png)

可以看到 AppService 是被 @Injectable 修饰的 class：

![](./image/第07章—使用多种Provider，灵活注入对象-2.png)

在 Module 的 providers 里声明：

![](./image/第07章—使用多种Provider，灵活注入对象-3.png)

这就是 provider。

其实这是一种简写，完整的写法是这样的：

![](./image/第07章—使用多种Provider，灵活注入对象-4.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-5.png)

如果不想用构造器注入，也可以属性注入：

![](./image/第07章—使用多种Provider，灵活注入对象-6.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-7.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-8.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-9.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-10.image#?w=1446&h=1080&s=136411&e=png&b=1c1c1c.png)

浏览器访问 http://localhost:3000 ，代码会在断点处断住。

![](./image/第07章—使用多种Provider，灵活注入对象-11.image#?w=1872&h=1310&s=328087&e=png&b=1c1c1c.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-12.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-13.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-14.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-15.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-16.png)

这个 useFactory 支持通过参数注入别的 provider：

![](./image/第07章—使用多种Provider，灵活注入对象-17.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-18.png)

在 return 那里打个断点。

可以看到，在调用 useFactory 方法的时候，Nest 就会注入这两个对象：

![](./image/第07章—使用多种Provider，灵活注入对象-19.png)

useFactory 支持异步：

![](./image/第07章—使用多种Provider，灵活注入对象-20.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-21.png)

这样就可以更灵活的创建注入对象。

此外，provider 还可以通过 useExisting 来指定别名：

![](./image/第07章—使用多种Provider，灵活注入对象-22.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-23.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-24.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-25.png)
...
![](./image/第07章—使用多种Provider，灵活注入对象-26.png)

这里是一个版本用了 Connection，一个版本用了 DataSource，通过 useExisting 起别名就可以兼容两者。

此外，如果觉得构造器注入写起来不方便，可以使用属性注入，效果一样：

![](./image/第07章—使用多种Provider，灵活注入对象-27.png)

![](./image/第07章—使用多种Provider，灵活注入对象-28.png)
