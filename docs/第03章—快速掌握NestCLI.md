...
![](./image/第03章—快速掌握NestCLI-7.png)

它会生成 module 的代码：

![](./image/第03章—快速掌握NestCLI-8.png)

还会自动在 AppModule 里引入：

![](./image/第03章—快速掌握NestCLI-9.png)
...
```
![](./image/第03章—快速掌握NestCLI-10.png)

同样，它也会更新到 module 的依赖里去。

生成 service 也是一样：

![](./image/第03章—快速掌握NestCLI-11.png)
...
![](./image/第03章—快速掌握NestCLI-12.png)

然后会让你选择是否生成 CRUD 代码：

![](./image/第03章—快速掌握NestCLI-13.png)

选择是。

然后就会生成整个模块的 CRUD + REST api 的代码：

![](./image/第03章—快速掌握NestCLI-14.png)

![](./image/第03章—快速掌握NestCLI-15.png)

当然，它同样会自动在 AppModule 引入：

![](./image/第03章—快速掌握NestCLI-16.png)

这就是 nest generate，可以快速生成各种代码：

![](./image/第03章—快速掌握NestCLI-17.png)
...
![](./image/第03章—快速掌握NestCLI-18.png)

它的实现原理很简单，就是模版引擎填充变量，打印成代码：

![](./image/第03章—快速掌握NestCLI-19.png)

nest generate 也有不少选项：

![](./image/第03章—快速掌握NestCLI-20.png)

--flat 和 --no-flat 是指定是否生成对应目录的：

![](./image/第03章—快速掌握NestCLI-21.png)

--spec 和 --no-spec 是指定是否生成测试文件：

![](./image/第03章—快速掌握NestCLI-22.png)

--skip-import 是指定不在 AppModule 里引入：

![](./image/第03章—快速掌握NestCLI-23.png)

也就是不生成这部分代码：

![](./image/第03章—快速掌握NestCLI-24.png)
...
![](./image/第03章—快速掌握NestCLI-25.png)

执行 nest build，会在 dist 目录下生成编译后的代码。

同样，它也有一些选项：

![](./image/第03章—快速掌握NestCLI-26.png)

--wepback 和 --tsc 是指定用什么编译，默认是 tsc 编译，也可以切换成 webpack。

这是 tsc 的编译产物：

![](./image/第03章—快速掌握NestCLI-27.png)

这是 webpack 的编译产物：

![](./image/第03章—快速掌握NestCLI-28.png)
...
![](./image/第03章—快速掌握NestCLI-34.png)

可以看到每次重新 build 了，并且用 node 把 main.js 跑了起来。

它有这些选项：

![](./image/第03章—快速掌握NestCLI-35.png)

--watch 是最常用的选项了，也就是改动文件之后自动重新 build：

![](./image/第03章—快速掌握NestCLI-36.png)

--debug 是启动调试的 websocket 服务，用来 debug。

![](./image/第03章—快速掌握NestCLI-37.png)
