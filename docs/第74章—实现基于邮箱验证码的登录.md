...
![](./image/第74章—实现基于邮箱验证码的登录-1.png)

![](./image/第74章—实现基于邮箱验证码的登录-2.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-3.png)

比较丑，但功能是没问题的：

![](./image/第74章—实现基于邮箱验证码的登录-4.png)
...
```
![](./image/第74章—实现基于邮箱验证码的登录-5.png)

在 main.ts 启用跨域，并且修改下端口号（因为前端项目开发服务也用这个端口号）：

![](./image/第74章—实现基于邮箱验证码的登录-6.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-12.png)

保存，nest 服务会自动重新跑：

![](./image/第74章—实现基于邮箱验证码的登录-13.png)

可以看到 user 表被创建了。

![](./image/第74章—实现基于邮箱验证码的登录-14.png)

这次我们手动插入下数据：

![](./image/第74章—实现基于邮箱验证码的登录-15.png)

输入内容后，点击 apply，可以看到生成的 sql：

![](./image/第74章—实现基于邮箱验证码的登录-16.png)
...
```
![](./image/第74章—实现基于邮箱验证码的登录-17.png)

![](./image/第74章—实现基于邮箱验证码的登录-18.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-19.png)

发送成功，邮箱里确实也收到了这封邮件：

![](./image/第74章—实现基于邮箱验证码的登录-20.png)

![](./image/第74章—实现基于邮箱验证码的登录-21.png)

回过头来看一下，我们现在是把邮箱相关信息直接写在代码里了。

![](./image/第74章—实现基于邮箱验证码的登录-22.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-23.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-24.png)

然后在 EmailService 里注入 ConfigService，从中读取配置：

![](./image/第74章—实现基于邮箱验证码的登录-25.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-26.png)

![](./image/第74章—实现基于邮箱验证码的登录-27.png)

说明配置正确读取出来了。

不过用了 .env 配置文件之后有个问题：

![](./image/第74章—实现基于邮箱验证码的登录-28.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-29.png)
...
npm run start:dev
```
![](./image/第74章—实现基于邮箱验证码的登录-30.png)

这时就可以在 dist 下看到这个文件了。

但现在你改了 src 下的 .env 之后，dist 下的 .env 不会跟着改，需要重新跑服务才可以。

![](./image/第74章—实现基于邮箱验证码的登录-31.png)

这时候可以加一个 watchAssets：

![](./image/第74章—实现基于邮箱验证码的登录-32.png)

然后再重新跑下服务。

![](./image/第74章—实现基于邮箱验证码的登录-33.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-34.png)

```javascript
const code = Math.random().toString().slice(2,8);
````
![](./image/第74章—实现基于邮箱验证码的登录-35.png)

然后在前端项目里调用下看看：

![](./image/第74章—实现基于邮箱验证码的登录-36.png)

通过 useForm 创建 form 实例，然后就可以通过 form.getFieldsValue 拿到表单值了：

![](./image/第74章—实现基于邮箱验证码的登录-37.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-38.png)

点击发送验证码，这个邮箱收到了一封验证码的邮件：

![](./image/第74章—实现基于邮箱验证码的登录-39.png)

然后我们来实现下登录。

登录就是根据用户填的信息去数据库匹配，如果匹配到了就查询出该用户的信息，放入 session 或者 jwt 里。

![](./image/第74章—实现基于邮箱验证码的登录-40.png)

验证用户身份的信息，可以是用户名 + 密码，也可以是邮箱 + 验证码。
    
用邮箱验证码验证用户身份的流程是这样的：

![](./image/第74章—实现基于邮箱验证码的登录-41.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-42.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-43.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-44.png)

输入邮箱地址，点击发送验证码。

邮箱收到了这个验证码：

![](./image/第74章—实现基于邮箱验证码的登录-45.png)

redis 里也保存了一份：

![](./image/第74章—实现基于邮箱验证码的登录-46.png)

接下来只要填入这个验证码，点击登录就可以了：

![](./image/第74章—实现基于邮箱验证码的登录-47.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-48.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-49.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-50.png)

![](./image/第74章—实现基于邮箱验证码的登录-51.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-52.png)

输入邮箱，点击发送验证码。

![](./image/第74章—实现基于邮箱验证码的登录-53.png)

收到了验证码邮件，并且 redis 里也存了这个 key：

![](./image/第74章—实现基于邮箱验证码的登录-54.png)

带上这个验证码请求：

![](./image/第74章—实现基于邮箱验证码的登录-55.png)

可以看到服务端通过了校验，并且从数据库中查询出了用户信息：

![](./image/第74章—实现基于邮箱验证码的登录-56.png)
...
![](./image/第74章—实现基于邮箱验证码的登录-57.png)
