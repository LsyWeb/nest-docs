...
![](./image/第60章—如何动态读取不同环境的配置-1.png)

应用启动的时候，我们指定了端口：

![](./image/第60章—如何动态读取不同环境的配置-2.png)
...
![](./image/第60章—如何动态读取不同环境的配置-3.png)
进入这个目录，安装 dotenv：
...
node index.js
```
![](./image/第60章—如何动态读取不同环境的配置-4.png)
...
![](./image/第60章—如何动态读取不同环境的配置-5.png)

如果你手动置顶了环境变量，那以手动指定的优先：

![](./image/第60章—如何动态读取不同环境的配置-6.png)
...
![](./image/第60章—如何动态读取不同环境的配置-7.png)
...
![](./image/第60章—如何动态读取不同环境的配置-8.png)
...
![](./image/第60章—如何动态读取不同环境的配置-9.png)
...
![](./image/第60章—如何动态读取不同环境的配置-10.png)
...
![](./image/第60章—如何动态读取不同环境的配置-11.png)
...
![](./image/第60章—如何动态读取不同环境的配置-12.png)

在 Controller 里取出来：

![](./image/第60章—如何动态读取不同环境的配置-13.png)

浏览器访问下：

![](./image/第60章—如何动态读取不同环境的配置-14.png)
...
![](./image/第60章—如何动态读取不同环境的配置-15.png)

同样，前面覆盖后面的。

改下 Controller：

![](./image/第60章—如何动态读取不同环境的配置-16.png)

浏览器访问下：

![](./image/第60章—如何动态读取不同环境的配置-17.png)
...
![](./image/第60章—如何动态读取不同环境的配置-18.png)

在 BbbModule 里注入下：

![](./image/第60章—如何动态读取不同环境的配置-19.png)

跑起来你会发现报错了：

![](./image/第60章—如何动态读取不同环境的配置-20.png)

这个模块找不到 ConfigModule。

这时候把 ConfigModule.forRoot 注册为全局模块就好了：

![](./image/第60章—如何动态读取不同环境的配置-21.png)

这样就可以在 BbbModule 读取到配置了：

![](./image/第60章—如何动态读取不同环境的配置-22.png)
...
![](./image/第60章—如何动态读取不同环境的配置-23.png)
...
![](./image/第60章—如何动态读取不同环境的配置-24.png)
...
![](./image/第60章—如何动态读取不同环境的配置-25.png)

之后动态返回模块定义：

![](./image/第60章—如何动态读取不同环境的配置-26.png)
