...
![](http://static.liushuaiyang.com/nest-docs/image/第05章-1.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第05章-2.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第05章-3.png)

选择一个 npm 包管理工具，然后 nest cli 会自动创建项目结构并安装依赖：

![](http://static.liushuaiyang.com/nest-docs/image/第05章-4.png)

然后进入这个目录，执行 npm run start，把服务跑起来：

![](http://static.liushuaiyang.com/nest-docs/image/第05章-5.png)

浏览器访问 http://localhost:3000 就可以看到 nest 服务返回的 hello world：

![](http://static.liushuaiyang.com/nest-docs/image/第05章-6.png)

我们看看代码里它是怎么创建对象的：

![](http://static.liushuaiyang.com/nest-docs/image/第05章-7.png)

它有一个 AppService 声明了 @Injectable，代表这个 class 可注入，那么 nest 就会把它的对象放到 IOC 容器里。

![](http://static.liushuaiyang.com/nest-docs/image/第05章-8.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第05章-9.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第05章-10.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第05章-11.png)

那么 nest 就会从 AppModule 开始解析 class 上通过装饰器声明的依赖信息，自动创建和组装对象。

![](http://static.liushuaiyang.com/nest-docs/image/第05章-12.png)

所以 AppController 只是声明了对 AppService 的依赖，就可以调用它的方法了：

![](http://static.liushuaiyang.com/nest-docs/image/第05章-13.png)
...
![](http://static.liushuaiyang.com/nest-docs/image/第05章-14.png)

会生成如下代码：

![](http://static.liushuaiyang.com/nest-docs/image/第05章-15.png)

用 nest cli 的 generate 命令生成一个模块。

会在 AppModule 里自动 imports 这个模块：

![](http://static.liushuaiyang.com/nest-docs/image/第05章-16.png)
...
```
![](http://static.liushuaiyang.com/nest-docs/image/第05章-17.png)

会生成 Service 的代码：

![](http://static.liushuaiyang.com/nest-docs/image/第05章-18.png)

并自动添加到 OtherModule 的 providers 中：

![](http://static.liushuaiyang.com/nest-docs/image/第05章-19.png)

我们改下 OtherService，添加一个方法：

![](http://static.liushuaiyang.com/nest-docs/image/第05章-20.png)

然后在 OtherModule 里 exports：

![](http://static.liushuaiyang.com/nest-docs/image/第05章-21.png)

那当 AppModule 引用了 OtherModule 之后，就可以注入它 exports 的 OtherService 了。

![](http://static.liushuaiyang.com/nest-docs/image/第05章-22.png)
...
npm run start:dev
```
![](http://static.liushuaiyang.com/nest-docs/image/第05章-23.png)


浏览器访问下，可以看到 AppModule 的 AppService 调用 OtherModule 的 OtherService 成功了：

![](http://static.liushuaiyang.com/nest-docs/image/第05章-24.png)
