...
![](./image/第68章—基于Redis实现分布式session-5.png)
...
```
![](./image/第68章—基于Redis实现分布式session-6.png)
...
![](./image/第68章—基于Redis实现分布式session-7.image#?w=722&h=196&e=webp&b=1f1f1f.png)
...
```
![](./image/第68章—基于Redis实现分布式session-8.png)
...
![](./image/第68章—基于Redis实现分布式session-9.png)
...
![](./image/第68章—基于Redis实现分布式session-10.png)
...
![](./image/第68章—基于Redis实现分布式session-11.png)

现在 getSession 返回的是 Record<string, any> 也就是对象类型，但并不知道有啥具体的属性。

![](./image/第68章—基于Redis实现分布式session-12.png)
...
![](./image/第68章—基于Redis实现分布式session-13.png)

传入类型参数之后，返回的就是该类型了：

![](./image/第68章—基于Redis实现分布式session-14.png)
...
![](./image/第68章—基于Redis实现分布式session-15.png)

我们自己实现的 session 就生效了：

![](./image/第68章—基于Redis实现分布式session-16.png)

在 Redis Insight 里可以看到 session 的值

![](./image/第68章—基于Redis实现分布式session-17.png)

而且这个 session 是支持分布式的。


![](./image/第68章—基于Redis实现分布式session-18.png)
