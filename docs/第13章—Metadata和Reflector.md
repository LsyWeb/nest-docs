...
![](./image/第13章—Metadata和Reflector-1.png)

Reflect.set 是设置对象属性值

![](./image/第13章—Metadata和Reflector-2.png)

Reflect.has 是判断对象属性是否存在

![](./image/第13章—Metadata和Reflector-3.png)

Reflect.apply 是调用某个方法，传入对象和参数

![](./image/第13章—Metadata和Reflector-4.png)

Reflect.construct 是用构造器创建对象实例，传入构造器参数

![](./image/第13章—Metadata和Reflector-5.png)
...
![](./image/第13章—Metadata和Reflector-6.png)
...
![](./image/第13章—Metadata和Reflector-7.png)
...
![](./image/第13章—Metadata和Reflector-8.png)

![](./image/第13章—Metadata和Reflector-9.png)

而且 @Controller 和 @Injectable 的装饰器也是这样实现的：

![](./image/第13章—Metadata和Reflector-10.png)

![](./image/第13章—Metadata和Reflector-11.png)
...
![](./image/第13章—Metadata和Reflector-12.png)

但是呢，ts 有一个编译选项叫做 emitDecoratorMetadata，开启它就会自动添加一些元数据。

开启之后再试一下：

![](./image/第13章—Metadata和Reflector-13.png)
...
![](./image/第13章—Metadata和Reflector-17.png)

这个装饰器的底层实现自然是 Reflect.defineMetadata：

![](./image/第13章—Metadata和Reflector-18.png)
...
![](./image/第13章—Metadata和Reflector-19.png)

在路由级别应用：

![](./image/第13章—Metadata和Reflector-20.png)

加个打印语句：

![](./image/第13章—Metadata和Reflector-21.png)

![](./image/第13章—Metadata和Reflector-22.png)

然后 nest start --watch 把服务跑起来。

浏览器访问：

![](./image/第13章—Metadata和Reflector-23.png)

可以看到 guard 和 interceptor 成功执行了：

![](./image/第13章—Metadata和Reflector-24.png)

然后我们用 @SetMetadata 在 controller 上加个 metadata：

![](./image/第13章—Metadata和Reflector-25.png)

在 guard 和 interceptor 就就可以这样取出来：

![](./image/第13章—Metadata和Reflector-26.png)

通过 ExecutationContext 取到目标 handler，然后注入 reflector，通过 reflector.get 取出 handler 上的 metadata。

interceptor 里也是这样，这里换种属性注入方式：

![](./image/第13章—Metadata和Reflector-27.png)

刷新下页面，就可以看到已经拿到了 metadata：

![](./image/第13章—Metadata和Reflector-28.png)
...
![](./image/第13章—Metadata和Reflector-29.png)

![](./image/第13章—Metadata和Reflector-30.png)

![](./image/第13章—Metadata和Reflector-31.png)

reflector 还有 3 个方法：

![](./image/第13章—Metadata和Reflector-32.png)
...
![](./image/第13章—Metadata和Reflector-33.png)

get 的实现就是 Reflect.getMetadata。

![](./image/第13章—Metadata和Reflector-34.png)

getAll 是返回一个 metadata 的数组。

![](./image/第13章—Metadata和Reflector-35.png)

getAllAndMerge，会把它们合并为一个对象或者数组。

![](./image/第13章—Metadata和Reflector-36.png)

getAllAndOverride 会返回第一个非空的 metadata。

我们试一下：

![](./image/第13章—Metadata和Reflector-37.png)

可以看到它们结果的区别：

![](./image/第13章—Metadata和Reflector-38.png)
