...
![](./image/第08章—全局模块和生命周期-2.png)

在 AaaModule 里指定 exports 的 provider：

![](./image/第08章—全局模块和生命周期-3.png)

然后在 BbbModule 里 imports：

![](./image/第08章—全局模块和生命周期-4.png)

这样就可以在 BbbModule 内注入 AaaService 了：

![](./image/第08章—全局模块和生命周期-5.png)
...
![](./image/第08章—全局模块和生命周期-7.png)

在 AaaModule 上加一个 @Global 的装饰器，然后在 BbbModule 里把 AaaModule 的 imports 去掉。

![](./image/第08章—全局模块和生命周期-8.png)

这样依然是可以注入的：

![](./image/第08章—全局模块和生命周期-9.png)

![](./image/第08章—全局模块和生命周期-10.png)
...
![](./image/第08章—全局模块和生命周期-12.png)

nest 提供了这样两个 interface：

![](./image/第08章—全局模块和生命周期-13.png)

![](./image/第08章—全局模块和生命周期-14.png)

在 controller、service、module 里分别实现它：

![](./image/第08章—全局模块和生命周期-15.png)

![](./image/第08章—全局模块和生命周期-16.png)

![](./image/第08章—全局模块和生命周期-17.png)

ddd 模块也是这样。

然后重新跑下服务，会看到这样的日志信息：

![](./image/第08章—全局模块和生命周期-18.png)

这就是 onModuleInit 和 onApplicationBootstrap 生命周期的调用顺序。

应用销毁的时候也同样有生命周期：

![](./image/第08章—全局模块和生命周期-19.png)
...
![](./image/第08章—全局模块和生命周期-20.png)

![](./image/第08章—全局模块和生命周期-21.png)
...
![](./image/第08章—全局模块和生命周期-22.png)

![](./image/第08章—全局模块和生命周期-23.png)

![](./image/第08章—全局模块和生命周期-24.png)

3s 后调用 app.close() 触发销毁（app.close() 只是触发销毁逻辑，但不会真正退出进程）

![](./image/第08章—全局模块和生命周期-25.png)

生命周期方法是这样的执行顺序：

![](./image/第08章—全局模块和生命周期-26.png)

而且所有的生命周期函数都是支持 async 的。

我们来看看 @nestjs/typeorm、@nestjs/mongoose 里都是怎么用的：

![](./image/第08章—全局模块和生命周期-27.png)

![](./image/第08章—全局模块和生命周期-28.png)
