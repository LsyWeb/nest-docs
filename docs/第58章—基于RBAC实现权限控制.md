...
![](./image/第58章—基于RBAC实现权限控制-1.png)

直接给用户分配权限。

而 RBAC 是这样的：

![](./image/第58章—基于RBAC实现权限控制-2.png)
...
```
![](./image/第58章—基于RBAC实现权限控制-3.png)

可以看到创建出的 database：

![](./image/第58章—基于RBAC实现权限控制-4.png)
...
![](./image/第58章—基于RBAC实现权限控制-6.png)

添加 User、Role、Permission 的 Entity：


![](./image/第58章—基于RBAC实现权限控制-7.png)
...
![](./image/第58章—基于RBAC实现权限控制-10.png)

![](./image/第58章—基于RBAC实现权限控制-11.png)

![](./image/第58章—基于RBAC实现权限控制-12.png)

![](./image/第58章—基于RBAC实现权限控制-13.png)

![](./image/第58章—基于RBAC实现权限控制-14.png)

还有外键：

![](./image/第58章—基于RBAC实现权限控制-15.png)

![](./image/第58章—基于RBAC实现权限控制-16.png)
...
![](./image/第58章—基于RBAC实现权限控制-17.png)
...
![](./image/第58章—基于RBAC实现权限控制-18.png)

服务端控制台打印了一堆 sql：

![](./image/第58章—基于RBAC实现权限控制-19.png)
...
![](./image/第58章—基于RBAC实现权限控制-20.png)

role 表：

![](./image/第58章—基于RBAC实现权限控制-21.png)

user 表：

![](./image/第58章—基于RBAC实现权限控制-22.png)

role_permission_relation 中间表：

![](./image/第58章—基于RBAC实现权限控制-23.png)

user_role_relation 中间表：

![](./image/第58章—基于RBAC实现权限控制-24.png)
...
![](./image/第58章—基于RBAC实现权限控制-25.png)

然后在 postman 里测试下：

ValidationPipe 不通过的时候，会返回错误信息：

![](./image/第58章—基于RBAC实现权限控制-26.png)

ValidationPipe 通过之后，就会执行 handler 里的方法：

![](./image/第58章—基于RBAC实现权限控制-27.png)

![](./image/第58章—基于RBAC实现权限控制-28.png)
...
![](./image/第58章—基于RBAC实现权限控制-29.png)

![](./image/第58章—基于RBAC实现权限控制-30.png)
...
![](./image/第58章—基于RBAC实现权限控制-31.png)

设置为全局模块，这样不用每个模块都引入。

然后在 UserController 里注入 JwtModule 里的 JwtService：

![](./image/第58章—基于RBAC实现权限控制-32.png)
...
![](./image/第58章—基于RBAC实现权限控制-33.png)
...
nest g resource bbb 
```
![](./image/第58章—基于RBAC实现权限控制-34.png)

![](./image/第58章—基于RBAC实现权限控制-35.png)

现在这些接口可以直接访问：

![](./image/第58章—基于RBAC实现权限控制-36.png)

![](./image/第58章—基于RBAC实现权限控制-37.png)
...
![](./image/第58章—基于RBAC实现权限控制-38.png)

但这时候会报错 user 不在 Request 的类型上。

扩展下就好了：

![](./image/第58章—基于RBAC实现权限控制-39.png)
...
![](./image/第58章—基于RBAC实现权限控制-40.png)

这样太麻烦了，这次我们全局加：

![](./image/第58章—基于RBAC实现权限控制-41.png)
...
![](./image/第58章—基于RBAC实现权限控制-42.png)

再访问下 aaa、bbb 接口：

![](./image/第58章—基于RBAC实现权限控制-43.png)

![](./image/第58章—基于RBAC实现权限控制-44.png)

但这时候你访问 /user/login 接口也被拦截了：

![](./image/第58章—基于RBAC实现权限控制-45.png)
...
![](./image/第58章—基于RBAC实现权限控制-49.png)

因为没有 require-login 的 metadata：

![](./image/第58章—基于RBAC实现权限控制-50.png)

而 aaa、bbb 是需要登录的：

![](./image/第58章—基于RBAC实现权限控制-51.png)

![](./image/第58章—基于RBAC实现权限控制-52.png)

因为它们包含 require-login 的metadata：

![](./image/第58章—基于RBAC实现权限控制-53.png)

然后我们登录下，带上 token 访问试试：

![](./image/第58章—基于RBAC实现权限控制-54.png)

![](./image/第58章—基于RBAC实现权限控制-55.png)
...
![](./image/第58章—基于RBAC实现权限控制-58.png)

可以看到打印了这个用户拥有的角色的所有 permission 信息：

![](./image/第58章—基于RBAC实现权限控制-59.png)

再增加个自定义 decorator：

![](./image/第58章—基于RBAC实现权限控制-60.png)
...
![](./image/第58章—基于RBAC实现权限控制-61.png)

在 PermissionGuard 里取出来判断：


![](./image/第58章—基于RBAC实现权限控制-62.png)
...
![](./image/第58章—基于RBAC实现权限控制-63.png)

带上 token 访问：

![](./image/第58章—基于RBAC实现权限控制-64.png)
...
![](./image/第58章—基于RBAC实现权限控制-65.png)
...
![](./image/第58章—基于RBAC实现权限控制-66.png)

我们再登录下张三账号：

![](./image/第58章—基于RBAC实现权限控制-67.png)

用这个 token 去访问下 bbb 接口，就能正常访问了：

![](./image/第58章—基于RBAC实现权限控制-68.png)

他是有这个权限的：

![](./image/第58章—基于RBAC实现权限控制-69.png)
