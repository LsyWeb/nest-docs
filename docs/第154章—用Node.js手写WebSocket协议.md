...
![](./image/第154章—用Node.js手写WebSocket协议-1.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-2.png)

随便找个有 websocket 的网站，比如知乎就有：

![](./image/第154章—用Node.js手写WebSocket协议-3.png)

过滤出 ws 类型的请求，看看这几个 header，是不是就是前面说的那些。

这个 Sec-WebSocket-Key 是 wk60yiym2FEwCAMVZE3FgQ==

![](./image/第154章—用Node.js手写WebSocket协议-4.png)

而响应的 Sec-WebSocket-Accept 是 XRfPnS+8xl11QWZherej/dkHPHM=

![](./image/第154章—用Node.js手写WebSocket协议-5.png)

我们算算看：

![](./image/第154章—用Node.js手写WebSocket协议-6.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-7.png)

全新的协议？那具体是什么样的协议呢？

这样的：

![](./image/第154章—用Node.js手写WebSocket协议-8.png)

大家习惯的 http 协议是 key:value 的 header 带个 body 的：

![](./image/第154章—用Node.js手写WebSocket协议-9.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-10.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-11.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-12.image#?w=1174&h=1116&s=184085&e=png&b=1f1f1f.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-13.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-14.png)

这 3 个 header 还有 101 状态码都是我们返回的。

message 里也可以看到发送的消息：

![](./image/第154章—用Node.js手写WebSocket协议-15.png)

再去服务端看看，也收到了这个消息：

![](./image/第154章—用Node.js手写WebSocket协议-16.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-17.png)

我们需要第一个字节的后四位，也就是 opcode。

这样写：

![](./image/第154章—用Node.js手写WebSocket协议-18.image#?w=696&h=134&s=29918&e=png&b=202020.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-19.png)

第一位是 mask 标志位，后 7 位是 payload 长度。

可以这样取：

![](./image/第154章—用Node.js手写WebSocket协议-20.image#?w=870&h=444&s=83058&e=png&b=1f1f1f.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-21.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-22.image#?w=844&h=682&s=133340&e=png&b=1f1f1f.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-23.png)

这样就拿到了 payload 的长度，然后再用这个长度去截取内容就好了。

但在读取数据之前，还有个 mask 要处理，这个是用来给内容解密的：

![](./image/第154章—用Node.js手写WebSocket协议-24.png)

读 4 个字节，就是 mask key。

再后面的就可以根据 payload 长度读出来。

![](./image/第154章—用Node.js手写WebSocket协议-25.image#?w=1274&h=1002&s=211039&e=png&b=1f1f1f.png)
```javascript
let realData = null;
...
![](./image/第154章—用Node.js手写WebSocket协议-26.image#?w=942&h=964&s=127832&e=png&b=1f1f1f.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-27.png)

而现在我们能正确解析出其中的数据：

![](./image/第154章—用Node.js手写WebSocket协议-28.png)

至此，我们 websocket 协议的解析成功了！

这样的协议格式的数据叫做 frame，也就是帧：

![](./image/第154章—用Node.js手写WebSocket协议-29.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-30.image#?w=1050&h=900&s=175121&e=png&b=1f1f1f.png)
```javascript
send(data) {
    let opcode;
    let buffer;
...
![](./image/第154章—用Node.js手写WebSocket协议-31.image#?w=1142&h=994&s=177010&e=png&b=1f1f1f.png)
```javascript
...
![](./image/第154章—用Node.js手写WebSocket协议-32.png)
...
![](./image/第154章—用Node.js手写WebSocket协议-33.png)

收到客户端消息后，每两秒回一个消息。


![](./image/第154章—用Node.js手写WebSocket协议-34.png)
