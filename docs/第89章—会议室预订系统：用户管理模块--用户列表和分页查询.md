...
![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-5.png)
...
![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-8.png)

然后把 LoginGuard 里的异常改成 UnLoginException

![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-9.png)

测试下：

![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-10.png)

现在当你不带 token 访问 /aaa 的时候，返回的就是自己定义的格式了。

搜索下之前代码里抛的异常：

![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-11.png)
...
![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-13.png)

这样，所有的代码都不用修改，返回的响应就已经变了：

![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-14.png)

![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-15.png)
...
![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-25.png)
...
![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-31.png)
...
![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-34.png)
...
![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-36.png)
...
![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-38.png)

当我传 nickName 包含“里”的时候，服务端查询到数据只有一条。

![](./image/第89章—会议室预订系统：用户管理模块--用户列表和分页查询-39.png)
