...
![](./image/第37章—一对一、join查询、级联方式-1.png)

它们之间是一对一的关系，这时就可以用外键来表示。

![](./image/第37章—一对一、join查询、级联方式-2.png)
...
![](./image/第37章—一对一、join查询、级联方式-3.png)

分别添加 id、name 列：

![image.png](./image/第37章—一对一、join查询、级联方式-4.png)

点击 apply，建表 sql 如下：

![](./image/第37章—一对一、join查询、级联方式-5.png)
...
![](./image/第37章—一对一、join查询、级联方式-6.png)
...
![](./image/第37章—一对一、join查询、级联方式-7.png)

指定外键 user\_id 关联 user 表的 id。

这里还要选择主表数据 update 或者 delete 的时候，从表怎么办：

![](./image/第37章—一对一、join查询、级联方式-8.png)

我们先用默认的。

点击 apply，生成的建表 sql 是这样的：

![](./image/第37章—一对一、join查询、级联方式-9.png)
...
![](./image/第37章—一对一、join查询、级联方式-10.png)

![](./image/第37章—一对一、join查询、级联方式-11.png)
...
![](./image/第37章—一对一、join查询、级联方式-12.png)
...
![](./image/第37章—一对一、join查询、级联方式-13.png)
...
![](./image/第37章—一对一、join查询、级联方式-14.png)
...
![](./image/第37章—一对一、join查询、级联方式-15.png)
...
![](./image/第37章—一对一、join查询、级联方式-16.png)

点击左上角按钮，新建一条 sql：

![](./image/第37章—一对一、join查询、级联方式-17.png)

查询的结果是这样的：

![](./image/第37章—一对一、join查询、级联方式-18.png)
...
![](./image/第37章—一对一、join查询、级联方式-19.png)
...
![](./image/第37章—一对一、join查询、级联方式-20.png)
...
![](./image/第37章—一对一、join查询、级联方式-21.png)
...
![](./image/第37章—一对一、join查询、级联方式-22.png)
...
![](./image/第37章—一对一、join查询、级联方式-23.png)
...
![](./image/第37章—一对一、join查询、级联方式-24.png)
...
![](./image/第37章—一对一、join查询、级联方式-25.png)

其实可选的值有 4 种：

![](./image/第37章—一对一、join查询、级联方式-26.png)
...
![](./image/第37章—一对一、join查询、级联方式-27.png)

右键选择 delete row：

![](./image/第37章—一对一、join查询、级联方式-28.png)

这时候会提示你更新失败，因为有外键的约束。

![](./image/第37章—一对一、join查询、级联方式-29.png)

点击 revert，回到之前的状态：

![](./image/第37章—一对一、join查询、级联方式-30.png)

然后更新 id 为 11，点击 apply：
![](./image/第37章—一对一、join查询、级联方式-31.png)

同样会提示你更新失败，因为有外键的约束：

![](./image/第37章—一对一、join查询、级联方式-32.png)

这就是 **RESTIRCT 和 NO ACTION 的处理逻辑：只要从表有关联记录，就不能更新 id 或者删除记录。**

我们手动把从表记录的关联去掉，也就是删除第一条记录的外键（按 delete 键）：

![](./image/第37章—一对一、join查询、级联方式-33.png)

点击 apply 应用这次改动。

然后再试下主表的更新：

![](./image/第37章—一对一、join查询、级联方式-34.png)

这次就更新成功了！

![](./image/第37章—一对一、join查询、级联方式-35.png)

再来试下删除：

![](./image/第37章—一对一、join查询、级联方式-36.png)

同样也成功了：

![](./image/第37章—一对一、join查询、级联方式-37.png)

这就是 RESTRICT 或者 NO ACTION，只有当从表没有关联的记录的时候，才能更新主表记录的 id 或者删除它。

我们再来试试 CASCADE：

![](./image/第37章—一对一、join查询、级联方式-38.png)

修改外键级联方式为 CASCADE，点击 apply。

先看一下现在 id\_card 表的数据：

![](./image/第37章—一对一、join查询、级联方式-39.png)

把 id 为 2 的 user 的 id 改为 22，点击 apply：

![](./image/第37章—一对一、join查询、级联方式-40.png)

再看下 id\_card 表的数据，你会发现 user\_id 跟着改了。

![](./image/第37章—一对一、join查询、级联方式-41.png)

然后把 id 为 22 的 user 删除掉，点击 apply：

![](./image/第37章—一对一、join查询、级联方式-42.png)

再看下 id\_card 表会发现那条 user\_id 为 22 的记录也没了。

![](./image/第37章—一对一、join查询、级联方式-43.png)

这就是级联方式为 **CASCADE 的处理逻辑：主表删除，从表关联记录也级联删除，主表 id 更新，从表关联记录也跟着更新。**

然后再试下 set null：

![](./image/第37章—一对一、join查询、级联方式-44.png)

修改之后点击 apply。

查询下现在的 id\_card 表的数据：

![](./image/第37章—一对一、join查询、级联方式-45.png)

把 user 表中 id 为 5 的记录 id 改为 55，点击 apply：

![](./image/第37章—一对一、join查询、级联方式-46.png)

这时候 id\_card 中那条记录的外键被置为 null 了：

![](./image/第37章—一对一、join查询、级联方式-47.png)
